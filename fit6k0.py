...
#     theorypopt = [0.000920, 10,20,0]
    ...
    def wrap_bigsbpl(ivar, f0, frev, trev,nu01, nu02 ):
        ...
        for tval,nuval in zip(t,nu):
            fpk = f0*(tval/t0)**a1
            nupk1 = nu01*(tval/t0)**b1
            nupk2 = nu02*(tval/t0)**b2
            # f = sbpl(amplitude=fpk, x_break=nupk, alpha_1=-c1, alpha_2=-c2, delta=d)
            # f = sbpl(amplitude=fpk, x_break=nupk, alpha_1=-c1, alpha_2=-c2, delta=d)
            res1 = dsbpl(nuval,fpk,nupk1,c1,c2,nupk2,c3,s)
            res.append(res1)
        return frev + np.array(res)
    initial_guess = [1e-3,5e-5, 3.5,9,30]
    bounds = [(1e-6,1),(3e-5,2),(3,4),(1,50),(20,300)]
    bounds0 = tuple([b[0] for b in bounds])
    bounds1 = tuple([b[1] for b in bounds])
    bounds = [bounds0,bounds1]
    curdata = plotdata
