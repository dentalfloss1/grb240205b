...
if freezeParams:
    ...
    def get_tbreak(ivar, f0, nu0_1, nu0_2, k=0):
        t0 = 1
        d=0.4
        s = 10
        a1 = -k/(2*(4-k))
...
#     theorypopt = [0.000920, 10,20,0]
    ...
    def wrap_bigsbpl(ivar, f0, nu01,nu02):
        ...
        return  f
    initial_guess = [200e-6,5,50]
    bounds = [(1e-6,5e-4),(1,300),(1,300)]
    bounds0 = tuple([b[0] for b in bounds])
    bounds1 = tuple([b[1] for b in bounds])
    bounds = [bounds0,bounds1]
    curdata = plotdata[plotdata['obsdate'] > 2]
    tdata = curdata['obsdate']
    nudata = curdata['freq']
    xdata = (tdata,nudata)
    ydata = curdata['flux']*1e-6
...
for band,ax in zip(bands,axs):
   ...
   #  popt, pcov = curve_fit(wrap_sbpl, xdata, ydata, p0=initial_guess,bounds=bounds)
    ...
    input = bigpopt + [0]
    t_break = get_tbreak((xline,nu),*bigpopt)
    # print(test_theory)
