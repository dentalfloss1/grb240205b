...
for band,ax in zip(bands,axs.flatten()):
    curdata = plotdata[(plotdata['band']==band) & (plotdata['flux'] > 0)]
    # print(curdata)
    ...
    for freq in np.sort(np.unique(curdata['freq']))[::-1]:
       if band in ['X']:
          ...
          #  ax.plot(xline,yline,alpha=0.5,color='black',ls=next(linestyle),label=f"{freq} GHz model, Thin Shell")
           ...
           limitdata = plotdata[(plotdata['freq']==freq) & (plotdata['flux']==-1)]
           if limitdata.size > 0:
               xlimit = limitdata['obsdate']
               ylimit = limitdata['rms']*3
               print(wrap_bigsbpl((xlimit,[freq]),*bigpopt)*1e6, "model at limit","|", ylimit.to_numpy()[0],"limit")
               ax.scatter(xlimit, ylimit, marker='v', color='black', label="3$\sigma$ limits")
       else:
